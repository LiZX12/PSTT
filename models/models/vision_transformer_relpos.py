...
class RelPosMlp(nn.Module):
    ...
    def forward(self, attn, shared_rel_pos: Optional[torch.Tensor] = None):
        return attn + self.get_bias()[:,:,:attn.shape[-2],:attn.shape[-1]]
...
class VisionTransformerRelPos(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=RelPosBlock,stride_size=16):
        """
        Args:
        ...
        self.patch_embed = embed_layer(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,stride_size=stride_size)
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        camera_id = [i for i in camera_id for j in range(x.shape[0] // B)]
        x = x + 3 * torch.cat([self.sie_embed[camera_id[i]].unsqueeze(0) for i in range(len(camera_id))], dim=0)
...
@register_model
def vit_relpos_base_patch16_224(pretrained=True, **kwargs):
    """ ViT-Base (ViT-B/16) w/ relative log-coord position, no class token
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False, fc_norm=True,num_classes=625, **kwargs)
    model = _create_vision_transformer_relpos('vit_relpos_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def vit_relpos_base_patch16_224_DEMO(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) w/ relative log-coord position, no class token
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False, fc_norm=True,num_classes=625, **kwargs)
