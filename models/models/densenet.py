...
class DenseLayer(nn.Module):
    def __init__(
            ...
            drop_rate=0., memory_efficient=False, dilation_size=1):
        ...
        self.add_module('conv2', nn.Conv2d(
            bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=dilation_size, bias=False,
            dilation=dilation_size)),
        # self.add_module('norm3', norm_layer(growth_rate)),
    ...
    def forward(self, x):  # noqa: F811
        ...
        # new_features = self.norm3(self.conv2(self.norm2(bottleneck_output)))
...
class DenseNet(nn.Module):
    ...
    def __init__(
            ...
            memory_efficient=False, aa_stem_only=True, cfg=None):
        ...
        for i, num_layers in enumerate(block_config):
            block = DenseBlock(
                ...
                drop_rate=drop_rate,
                memory_efficient=memory_efficient,
                # dilation_size=[1, 2, 3, 4][i]
                dilation_size=[1, 1, 1, 1][i]
            )
    ...
    def forward(self, x, sn_type=0):
        for i in range(len(self.features)):
            x = self.features[i](x)
        return self.global_pool(x)
        # if self.cfg.MODEL.LR_TYPE == "FT" or (self.cfg.MODEL.LR_TYPE == "SN" and sn_type != 0):
        #     x = x1
        #     for i in range(len(self.features)):
        #         x = self.features[i](x)
        #     return self.global_pool(x)
        # elif self.cfg.MODEL.LR_TYPE == "SN":
        #     for layer in self.features:
        #         x1 = layer(x1)
        #         x2 = layer(x2)
        #     return [self.global_pool(x1), self.global_pool(x2)]
